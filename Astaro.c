...
int list_find(Case* L,int x,int y)//cette fonction envoie 1 si le point (x,y) est déjà dans la liste L 0 sinon
{
  Case* Liste=L;
  while(Liste!=NULL)
    {
...
Case* list_add(Case* L, Case* nouveau)
{

  if(list_find(L,nouveau->x,nouveau->y)==0)
    {
...
Case* delete (Case* element,int* x, int* y, int* cost, int* heuristique)
{
  ...
  element->next=delete(element->next,x,y,cost,heuristique);
  return element;
...
void afficher(Case* element) // permet d'afficher
{
  Case* copie=element;

  while(copie!=NULL)
    {
      printf("x:%d y:%d xp:%d yp:%d cost:%d heuristique:%d\n",copie->x,copie->y,copie->xp, copie->yp,copie->cost,copie->heuristique);
      copie=copie->next;
...
Case*  meilleur_chemin(t_labyrinthe* current)
{
  ...
  while(openlist!=NULL) // tant que l'openlist n'est pas vide si la case est vide et qu'on ne l'a pas encore exploré on l'ajoute à l'openlist
	...
	break;
      ...
      if(current->lab[((current->position_y-1+current->sizeX)%current->sizeX)*current->sizeX + current->position_x]==0) // si la case du haut est vide
        {
	  int h = manhattan(us_x, (us_y-1+current->sizeX)%current->sizeX, tresor_x, tresor_y);
	  nouvelle_case(copie,us_x,(us_y-1+current->sizeX)%current->sizeX,us_x,us_y,cost+1,h); // on rajoute dans l'openlist les coordonnées du point de départ

	  openlist=list_add(openlist,copie);


        }
      printf("Openlist add 1\n");
      afficher(openlist);
      copie=annule();

      if(current->lab[(current->position_y+1)%(current->sizeX) *current->sizeX + current->position_x]==0) // si la case du bas
        {

	  int h = manhattan(us_x, (us_y+1)%(current->sizeX), tresor_x, tresor_y);
	  nouvelle_case(copie,us_x,(us_y+1)%(current->sizeX),us_x,us_y,cost+1,h); // on rajoute dans l'openlist les coordonnées du point de départ
      ...
      afficher(openlist);
      copie=annule();
      if(current->lab[(current->position_y)*current->sizeX + (current->position_x-1+current->sizeY)%current->sizeY]==0) // si la case de gauche
        {

	  int h = manhattan((current->position_x-1+current->sizeY)%current->sizeY, us_y, tresor_x, tresor_y);
	  nouvelle_case(copie,(current->position_x-1+current->sizeY)%current->sizeY,us_y,us_x,us_y,cost+1,h); // on rajoute dans l'openlist les coordonnées du point de départ
      ...
      if(current->lab[(current->position_y) *current->sizeX + (current->position_x+1)%(current->sizeY)]==0) // droite
        {

	  int h = manhattan((us_x+1)%(current->sizeY), us_y, tresor_x, tresor_y);
	  nouvelle_case(copie,(us_x+1)%(current->sizeY),us_y,us_x,us_y,cost+1,h); // on rajoute dans l'openlist les coordonnées du point de départ
      ...
      closelist=list_add(closelist,small); // va ajouter à closelist le minimum de openlist
      // quand on ajoute un element à la closelist on la supprime de l'openlist
      //afficher(openlist);
      openlist=delete(openlist,&min(openlist)->x,&min(openlist)->y,&min(openlist)->cost,&min(openlist)->heuristique);
      printf("Openlist after del\n");
      afficher(openlist);
...
int main()
{
  ...
  connectToServer( "pc4023.polytech.upmc.fr", 1234,"Polyuser0144153");


  /* wait for a game, and retrieve informations about it */

  player=mylabyrinthe(Laby);
  meilleur_chemin(Laby); // pour openlist
