...
Case* extraire (Case* element,int* x, int* y, int* cost, int* heuristique)
{
    ...
    if(L!=NULL)
    {
        Case* prochain = element -> next; // on crée un pointeur prochain sur l'élément suivant de la chaine
        free(element); // on libère le premier élément à l'aide de free car on a supprimé son adresse
        return (prochain);
...
void meilleur_chemin(t_labyrinthe* current, Case* tmp)
{
    ...
    nouvelle_case(openlist,us_x,us_y,0,heuristique); // on rajoute dans l'openlist les coordonnées du point de départ
    list_add(openlist,tmp);
    ...
    while(openlist->next!=NULL) // tant que l'openlist n'est pas vide si la case est vide et qu'on ne l'a pas encore exploré on l'ajoute à l'openlist
    {
        ...
        if(current->lab[current->position_y-1*current->sizeX + current->position_x]==0) // si la case du haut est vide
        {
            ...
            nouvelle_case(tmp,us_x,us_y-1,cost+1,h); // on rajoute dans l'openlist les coordonnées du point de départ
            list_add(openlist, tmp);
        ...
        list_add(closelist,min(openlist)); // va ajouter à closelist le minimum de openlist
    }
...
int main()
{
    ...
    meilleur_chemin(Laby, openlist); // pour openlist
