...
Case* build_chemin(Case* closelist)
{
  Case* tmp=closelist;
  int size=0; 
  int i=0,j=0;
  int tab[100][100]; //on stock le chemin dans un tableau
  
  Case* last=get_last(closelist); 

  tmp=delete(tmp,last); // pour ne pas mettre le premier élément 
  ...
  while(openlist!=NULL) // tant que l'openlist n'est pas vide si la case est vide et qu'on ne l'a pas encore exploré on l'ajoute à l'openlist
    {
      ...
      if(small->x==tresor_x && small->y==tresor_y)
        {
	  Case* chemin=build_chemin(closelist);
	  return chemin;
...
void make_move(t_labyrinthe* current, t_move *move, Case* validite)
{
...
int main()
{
  ...
  Case* valide=meilleur_chemin(Laby); 
