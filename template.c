...
void remplissage(t_labyrinthe* current)
{
    ...
    if(current->lab[current->position_y-1*current->sizeX + current->position_x]==0) // si la case du haut est vide
    {
        ...
        current->lab[current->position_y-1*current->sizeX + current->position_x]==2; // puis on augmente la valeur de deux dans le labyrinthe car 1 correspond déjà à un mur dans le jeu
    ...
    if(current->lab[(current->position_y+1)%(current->sizeY) *current->sizeX + current->position_x]==0) // si la case du bas
    {
        ...
        current->lab[current->position_y+1*current->sizeX + current->position_x]==2;
    ...
    if(current->lab[(current->position_y) *current->sizeX + current->position_x-1]==0) // si la case de gauche
    {
        ...
        current->lab[current->position_y*current->sizeX + current->position_x-1]==2;
    }
...
void make_move(t_labyrinthe* current, t_move *move)
{
    ...
    while(test!=1)
    {
        ...
        while(current->position_x!=current->position_tresor_x && current->position_y!=current->position_tresor_y) // tant qu'on est pas arrivés au trésor
        {
            ...
            if(current->lab[deplacement*current->sizeX + current->position_x]==2) // si la case est dans l'openlist et qu'il ne s'agit donc pas d'un mur
            {
            ...
            if(current->lab[(current->position_y) *current->sizeX + deplacement]==2 && current->lab[(current->position_y) *current->sizeX + deplacement]<min)
            {
            ...
            if(current->lab[(current->position_y) *current->sizeX + (current->position_x+1)%(current->sizeX)]==2 && current->lab[(current->position_y) *current->sizeX + (current->position_x+1)%(current->sizeX)]<min)
            {
            ...
            if(current->lab[(current->position_y+1)%(current->sizeY) *current->sizeX + current->position_x]==2&&current->lab[(current->position_y+1)%(current->sizeY) *current->sizeX + current->position_x]<min)
            {
...
int main()
{
    ...
    remplissage(player); // pour openlist
