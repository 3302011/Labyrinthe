...
{ 
  ...
  while(test!=1)
      ...
      if(move->type==8)
	{
	 ...
	 current->lab[move->value*current->sizeX + current->sizeX-1]=Laby;
	 ...
	 current->lab[move->value*current->sizeX+ boucle]=current->lab[move->value*current->sizeX +boucle-1];
			...
			current->lab[move->value*current->sizeX]=Laby;
                 printLabyrinth(Laby);
          energy=energy-5; 
          test=1; 
      ...
      if(move->type==2)
      {
		 ...
		 current->lab[(current->sizeY-1)*current->sizeX + move->value]=Laby;
                printLabyrinth(Laby);
  ...
  do {
	...
	if(energy<5){
	...
	else
	  {
		...
		if(move.type==2 && Laby->lab[(Laby->sizeY-1)*Laby->sizeX + move.value])
		  {
		    move.type=ROTATE_COLUMN_UP;
		    energy=energy-5; 
		  }

		else if(move.type==3 && Laby->lab[move.value])
		  {
		    move.type= ROTATE_COLUMN_DOWN; 
		    energy=energy-5; 
		  }
