...
int mylabyrinthe(t_labyrinthe* current) 
{
  ...
  int sizeX,sizeY;
  char labName[50];
  waitForLabyrinth("DO_NOTHING timeout=100 rotate=True", labName, &sizeX, &sizeY);
  char* labData;
  ...
  current->position_adversaire_x=sizeX; 
...
void make_move(t_labyrinthe* current, t_move *move) // prend en entrée la structure ainsi que t_move qui nous permet d'utiliser MOVE_UP/RIGHT/DOWN/LEFT
{ 
  ...
  while(test!=1)
    {
      move->type=rand()%5 +4; // Pour générer un nombre aléatoire entre 4 et 8 
	...
	{  
      ...
      printf("vide ou pas:%d position_x: %d position_y: %d",current->lab[(current->position_y*current->sizeX+current->position_x)], current->position_x, current->position_y); 
    }


}

void rotate(t_move move, t_labyrinthe* Laby)
{
     
  if(move.type==0 && move.value==Laby->position_y)
    {
      if(Laby->position_x==0)  
	Laby->position_x=Laby->sizeX-1;
  
      else 
	Laby->position_x=Laby->position_x-1;  

    }              

  if(move.type==1 && move.value==Laby->position_y)
    {
      if(Laby->position_x==Laby->sizeX-1)
	Laby->position_x=0; 
      else 
	Laby->position_x=Laby->position_x+1; 
  ...
  if(move.type==2 && move.value==Laby->position_x)
 
    {
      if(Laby->position_y==0)
	Laby->position_y=Laby->sizeY-1; 

      else
	Laby->position_y=Laby->position_y-1; 

    }


  if(move.type==3 && move.value==Laby->position_x)
 
    {
      if(Laby->position_y==Laby->sizeY-1)
	Laby->position_y=0; 
...
int main()
{
  ...
  player=mylabyrinthe(Laby); // mylabyrinthe renvoie 0 si c'est à nous de jouer, 1 sinon. Ceci permet de jouer alténativement
  ...
  do {
	...
	if(Laby->energy<5){
	  move.value=0; // par sécurité 
	  make_move(Laby,&move); // ceci est la fonction que nous avons crée qui permet de faire bouger le joueur
          Laby->energy=Laby->energy+1; 
	  player=1; 
	  ...
	  {
	    move.type=rand()%9; // pour générer un nombre entre 0 et 8 

	    if(move.type==0 || move.type==1)
	      move.value=rand()%Laby->sizeY;
 
	    else if (move.type==2 || move.type==3)
	      move.value=rand()%Laby->sizeX;          
		...
		if(move.type>=4 && move.type<=8){
		  ...
		  player=1; 
                ...
                Laby->energy=Laby->energy-5;                 

                if(move.type==0 && Laby->lab[move.value*Laby->sizeX + Laby->sizeX-1]==0)
		  {
		    ...
		    tmp=Laby->lab[(move.value)*(Laby->sizeX)];      
		    boucle=0; 
		    while(boucle<(Laby->sizeX)-1)
		      {
			Laby->lab[(move.value)*(Laby->sizeX )+ boucle]=Laby->lab[(move.value)*(Laby->sizeX) +boucle+1];
			boucle++;
		      }
		    Laby->lab[(move.value)*(Laby->sizeX) + Laby->sizeX-1]=tmp;
		    printf("x:%d,y:%d",Laby->position_x,Laby->position_y); 
		    player=1; 
		  }

		else if( move.type==1 && Laby->lab[move.value*Laby->sizeX]==0)
		  {
		    ...
		    Laby->energy=Laby->energy-5;
                    ...
                    tmp=Laby->lab[(move.value)*(Laby->sizeX) + Laby->sizeX-1];
		    boucle=Laby->sizeX-1; 
		    ...
		    Laby->lab[(move.value)*(Laby->sizeX)]=tmp;
		    printf("x:%d,y:%d",Laby->position_x,Laby->position_y); 
		...
		else if(  move.type==2 && Laby->lab[(Laby->sizeY-1)*Laby->sizeX + move.value]==0)
		  {
		        
                    rotate(move,Laby);  
                    move.type=ROTATE_COLUMN_UP;
                    
                    tmp=Laby->lab[move.value];
		    boucle= 0;
		    while(boucle< (Laby->sizeY)-1)
		      {
			Laby->lab[boucle*(Laby->sizeX) + (move.value)]=Laby->lab[(boucle+1)*(Laby->sizeX) + move.value];
			boucle++; 
		      }
		    Laby->lab[(Laby->sizeY-1)*(Laby->sizeX) + (move.value)]=tmp;
		    printf("x:%d,y:%d",Laby->position_x,Laby->position_y); 
		    Laby->energy=Laby->energy-5; 
                    player=1; 
		...
		else if(move.type==3 && Laby->lab[move.value]==0)
		  {
		       
                    rotate(move,Laby);  
                    move.type= ROTATE_COLUMN_DOWN; 
                    
		    tmp=Laby->lab[(Laby->sizeY-1)*(Laby->sizeX) + move.value];
		    boucle=Laby->sizeY-1;
		    while(boucle>0)
		      {
			Laby->lab[boucle*(Laby->sizeX) + move.value]=Laby->lab[(boucle-1)*(Laby->sizeX) + move.value];
