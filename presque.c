...
void rotate(t_move move, t_labyrinthe* Laby)
{
  ...
  if(move.type==0 && move.value==Laby->position_y)
    {
      Laby->position_x=(Laby->sizeX+Laby->position_x-1)%Laby->sizeX; 
	...
	if(Laby->energy<5){
	...
	else
	  {
		...
		Laby->energy=Laby->energy-5; 
                ...
                if( Laby->energy>5 && move.type==0 && Laby->lab[move.value*Laby->sizeX + Laby->sizeX-1]==0)
		  {
		...
		else if(Laby->energy>5 && move.type==1 && Laby->lab[move.value*Laby->sizeX]==0)
		  {
		...
		else if( Laby->energy>5 && move.type==2 && Laby->lab[(Laby->sizeY-1)*Laby->sizeX + move.value]==0)
		  {
		...
		else if(Laby->energy >5 && move.type==3 && Laby->lab[move.value]==0)
		  {
