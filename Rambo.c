  ...
  do {
	...
	if(energy<5){
	...
	else
	  {
		...
		if(move.type==1 && Laby->lab[move.value*Laby->sizeX]==0)
		  {
		    move.type= ROTATE_LINE_RIGHT;
		    energy=energy-5; 
		  } 

		if(move.type==2 && Laby->lab[(Laby->sizeY-1)*Laby->sizeX + move.value]==0)
		  {
